...
```bash
docker compose -f my_local.yml run --rm django python manage.py makemessages --all --no-location
```
...
```bash
docker compose -f my_local.yml run --rm django python manage.py compilemessages
```
